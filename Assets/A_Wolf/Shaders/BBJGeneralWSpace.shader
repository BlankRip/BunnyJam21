Shader "BBJ/GeneralWSpace"
{
    Properties
    {
        _Color ("Color", Color) = (1,1,1,1)
        _MainTex ("Albedo (RGB)", 2D) = "white" {}
        _TexScale("Tex Scale", range(0,1)) = 0.2
    }
    SubShader
    {
        ...
        void surf (Input IN, inout SurfaceOutput  o)
        {
            ...
            fixed4 c = 
            (
            tex2D (_MainTex, fixed2(IN.worldPos.x,IN.worldPos.z) * _TexScale) * abs(o.Normal.g) + 
            tex2D (_MainTex, fixed2(IN.worldPos.z,IN.worldPos.y) * _TexScale) * abs(o.Normal.r) +
            tex2D (_MainTex, fixed2(IN.worldPos.x,IN.worldPos.y) * _TexScale) * abs(o.Normal.b)
            ) * _Color;
            o.Albedo = c.rgb;
            wavePos = IN.worldPos;
            viewDir = IN.viewDir;
